...
def main():
 ...
 out={'schema':'main_nersc_source_alignment_v10','status':'PASS' if all(x['match'] for x in rows) else 'BLOCKED','github_main_commit':a.commit,'n_files':len(rows),'n_matches':sum(x['match'] for x in rows),'n_mismatches':sum(not x['match'] for x in rows),'mismatches':[x for x in rows if not x['match']],'files':rows,'created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat()};a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({'status':out['status'],'n_files':len(rows),'mismatches':out['n_mismatches']}))
