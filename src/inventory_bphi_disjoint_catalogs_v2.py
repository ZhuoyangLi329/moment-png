...
def inventory_node(root,node):
 ...
 for rid in ids:
  ...
  rows.append({'realization':rid,'positions':pos.exists(),'masses':mass.exists(),'metadata':meta.exists(),'metadata_sha256':sha(meta) if meta.exists() else None,'selection_certified':bool(pos.exists() and mass.exists() and meta.exists() and md.get('mass_threshold_msun_h')==1e13 and md.get('snapnum')==2 and md.get('redshift')==1.0 and md.get('boxsize_mpc_h')==1000.0 and md.get('position_dtype')=='float32')})
