...
def main():
 ...
 p22=np.interp(kk,ku,p22,left=0,right=p22[-1]); p13=np.interp(kk,ku,p13,left=0,right=p13[-1]); dl=np.interp(kk,ku,dloop,left=0,right=dloop[-1]); ss=np.arange(40.,300.1,20.); rows=[]
 def proj(val,s):
 ...
 for fnl in [0.,-100.,100.]:
  tree=(a.b1+fnl*a.bphi/np.maximum(abs(M),1e-30))**2*P; comps={'tree':tree,'P22':p22,'P13':p13,'PNG_loop_response':fnl*dl,'total':tree+p22+p13+fnl*dl}; pred={key:[proj(val,s) for s in ss] for key,val in comps.items()}; rows.append({'fNL':fnl,'predictions':pred})
