...
def main():
 ...
 bphi_value=cal.get('bphi_conditional',cal.get('bphi',float('nan')))
 bphi_sigma=cal.get('bphi_sigma_statistical_conditional',cal.get('bphi_sigma_statistical',float('nan')))
 finite=all(math.isfinite(float(x)) for x in (bphi_value,bphi_sigma))
 checks.append({'name':'finite_fit_and_error','status':'PASS' if finite and float(bphi_sigma)>0 else 'BLOCKED'})
