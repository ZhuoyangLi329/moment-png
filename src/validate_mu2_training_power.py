#!/usr/bin/env python3
"""Leakage-free training Ph(k) calibration diagnostic for held-out Gaussian mu2."""
...
def main():
    ...
    for node in NODES:
        ...
        pred=np.array([float(mu2_gaussian_discrete(kv,pk,float(s),width=20.,boxsize=1000.,cell=15.625,nmesh=64,power_convention='mesh')[0]) for s in ss]); z=np.asarray(d['mu2_gaussian'],float)[hold]; mean=z.mean(0); cov_real=np.cov(z,rowvar=False,ddof=1); std_real=np.sqrt(np.diag(cov_real)); cov_mean=cov_real/len(z); se=np.sqrt(np.diag(cov_mean)); r=mean-pred; pull=r/np.maximum(se,1e-30); chi=float(r@np.linalg.pinv(cov_mean,rcond=1e-10)@r)
        rows[node]={'fNL':float(d['fNL']),'n_train':int(len(train)),'n_holdout':int(len(hold)),'prediction_mu2_gaussian':pred.tolist(),'mean_nbody_mu2_gaussian':mean.tolist(),'std_realization':std_real.tolist(),'se_mean':se.tolist(),'covariance_realization':cov_real.tolist(),'covariance_mean':cov_mean.tolist(),'pull':pull.tolist(),'rms_pull':float(np.sqrt(np.mean(pull*pull))),'chi2_dof':chi/len(r),'coverage_abs_pull_le_2':float(np.mean(np.abs(pull)<=2)),'training_P_h_only':True}
    out={'status':'PASS','schema':'mu2_training_ph_calibration_holdout_v2','nmesh':64,'s':ss.tolist(),'nodes':rows,'scope':'training-only mean Ph(k) exact-discrete Gaussian mu2 projection to held-out mu2G; diagnostic only','policy':{'train_ids':'real000-real069','holdout_ids':'real070-real099','heldout_response_fit':False,'production_promotion':False,'errorbar':'realization scatter; covariance_mean=covariance_realization/n_holdout'},'warning':'This is the Gaussian component only; contact, clustered halo, and primordial four-point terms are omitted.'}
