...
def main():
    ...
    artifact_checks=[]
    for ref, hkey in [('source_alignment','source_alignment_sha256'),('stage_progress','stage_progress_sha256'),('stage8_holdout_report','stage8_holdout_report_sha256')]:
        path=r/str(prov.get(ref,'')) if prov.get(ref) else None; expected=str(prov.get(hkey,'')); actual=hashlib.sha256(path.read_bytes()).hexdigest() if path and path.exists() else None; artifact_checks.append({'ref':ref,'path':str(path) if path else None,'expected_sha256':expected or None,'actual_sha256':actual,'match':bool(actual and expected and actual==expected)})
    checks.append({'name':'provenance_artifacts','status':'PASS' if all(x['match'] for x in artifact_checks) else 'BLOCKED','artifacts':artifact_checks})
    ...
    report_path=next((r/'results'/f'stage8_holdout_validation_report_v{i}.json' for i in [7,6,5,4,3,2,1] if (r/'results'/f'stage8_holdout_validation_report_v{i}.json').exists()),r/'results/stage8_holdout_validation_report_v1.json'); report=read(report_path); report_ok=bool(report and report.get('schema') in ('stage8_holdout_validation_report_v1','stage8_holdout_validation_report_v2','stage8_holdout_validation_report_v3','stage8_holdout_validation_report_v4','stage8_holdout_validation_report_v5','stage8_holdout_validation_report_v6','stage8_holdout_validation_report_v7') and report.get('status')=='REJECTED_DIAGNOSTIC' and report.get('config_sha256')==prov.get('frozen_config_sha256')); checks.append({'name':'stage8_holdout_report','status':'PASS' if report_ok else 'BLOCKED','evidence':str(report_path) if report_path.exists() else None,'diagnostic_status':(report or {}).get('status'),'schema':(report or {}).get('schema')})
    stage_path=next((r/'results'/f'stage_progress_v{i}.json' for i in range(30,0,-1) if (r/'results'/f'stage_progress_v{i}.json').exists()), r/'results/stage_progress_v2.json'); stage=read(stage_path); stages=(stage or {}).get('stages',{}); scientific=[]
