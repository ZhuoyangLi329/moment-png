...
def main():
    ...
    report_path=next((r/'results'/f'stage8_holdout_validation_report_v{i}.json' for i in [4,3,2,1] if (r/'results'/f'stage8_holdout_validation_report_v{i}.json').exists()),r/'results/stage8_holdout_validation_report_v1.json'); report=read(report_path); report_ok=bool(report and report.get('schema') in ('stage8_holdout_validation_report_v1','stage8_holdout_validation_report_v2','stage8_holdout_validation_report_v3','stage8_holdout_validation_report_v4') and report.get('status')=='REJECTED_DIAGNOSTIC' and report.get('config_sha256')==prov.get('frozen_config_sha256')); checks.append({'name':'stage8_holdout_report','status':'PASS' if report_ok else 'BLOCKED','evidence':str(report_path) if report_path.exists() else None,'diagnostic_status':(report or {}).get('status'),'schema':(report or {}).get('schema')})
