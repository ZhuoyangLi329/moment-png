...
def main():
    ...
    align_path=next((r/'results'/f'main_nersc_source_alignment_v{i}.json' for i in range(20,0,-1) if (r/'results'/f'main_nersc_source_alignment_v{i}.json').exists()),None); align=read(align_path) if align_path else None; checks.append({'name':'source_alignment','status':'PASS' if align and align.get('status')=='PASS' and not align.get('mismatches') else 'BLOCKED','evidence':str(align_path) if align_path else None,'n_files':(align or {}).get('n_files')})
