...
The NERSC metadata-checked read passed with 100 realizations per node, 14 frozen shells, 59 k bins, and fNL labels `[0,-100,100]`. It also read every selected metadata JSON and verified the `1e13 Msun/h` mass threshold, snapshot 2, node cosmology, and selected halo count. The compressed unified arrays use node order `[fiducial, LC_m, LC_p]`; source SHA256 values and halo-count arrays are recorded in `results/nbody_unified_v3.npz.json`.
